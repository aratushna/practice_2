...
namespace Kse.Algorithms.Samples
{
    ...
    public class MapGenerator
    {
        ...
        {
            var next = GetNextEmpty(); // Ïîëó÷àåì êîîðäèíàòû ñëåäóþùåé ïóñòîé òî÷êè (Space " ")

            var trafficRandom = new Random(options.TrafficSeed);
            while (next.HasValue) // öèêë ðàáîòàåò ïîêà íå áóäåò null
            {
            ...
            void PaintTrafficDfs(Point point, int depth, int value)
            {
                ...
                stack.Push(point); // Äîáàâëÿåì â ñòåê, äîáàâëÿåì ñâåðõó
       
                while (stack.Count > 0 && depth > 0)
                {
                    var next = stack.Pop(); // Ïîëó÷àåì ýëåìåíò è ñðàçó óäàëÿåì ýëåìåíò èç ñòåêà
                    ...
                    void Visit(Point point)
                    {
                        ...
                        depth -= 1; // Â -= A ýêâèâàëåíòíî B = B - A
                        visited.Add(point);
                    }
                }
            }
        ...
        public List<Point> GetNeighbours(int column, int row, string[,] maze, int offset = 2,
            bool checkWalls = false)
        {
            ...
            void TryAddWithOffset(int offsetX, int offsetY) // ïåðåäàåì ñìåùåíèÿ ïî x,y 
            { 
